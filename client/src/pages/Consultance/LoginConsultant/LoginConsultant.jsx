...
function LoginConsultant() {
    ...
    const Toast = Swal.mixin({
      ...
      const handleSubmit = async (event) => {
        ...
        try {
            const res = await axios.post('http://localhost:5000/api/auth/signinConsultant/', UserData)
            console.log(res.data)
            if(res.status == 200){
                navigate("/consultportfolio")
            }
            Cookies.set("consultant-auth", res.data['token'])
        } catch(error) {
            console.log(error);
            Toast.fire({
                icon: 'error',
        ...
        <div className="login-layout grid grid-cols-2 gap-4 bounce">
                ...
                <div className='relative flex items-center  '>
                    <form onSubmit={handleSubmit} className="login-form absolute text-center">
                        ...
                        <p className='login-label'>Consultant Login</p>
